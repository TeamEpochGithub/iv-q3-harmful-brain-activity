...
class ComplexModel(nn.Module):
    """Class for a complex model with convolutional and linear layers."""

    def __init__(self, input_size: int, hidden_size: int, num_classes: int, dropout_rate: float = 0.5) -> None:
...
class _WaveBlock(nn.Module):
    ...
    def __init__(
        ...
        conv_module: type[nn.Module] | None,
    ) -> None:
        super().__init__()
        ...
        nn.init.zeros_(self.in_conv.bias)  # type: ignore[arg-type]
...
class _GatedTCN(nn.Module):
    ...
    def __init__(
        ...
        conv_module: type[nn.Module] | None = None,
    ) -> None:
        super().__init__()
        ...
        if conv_module is None:
            self.filt = nn.Conv2d(
                in_channels=in_dim,
                out_channels=h_dim,
                kernel_size=(1, kernel_size),  # type: ignore[arg-type]
            ...
            self.filt = conv_module(  # type: ignore[assignment]
                in_channels=in_dim,
                out_channels=h_dim,
                kernel_size=kernel_size,
                dilation=dilation_factor,
            )
            self.gate = conv_module(  # type: ignore[assignment]
                in_channels=in_dim,
                out_channels=h_dim,
                kernel_size=kernel_size,
                dilation=dilation_factor,
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        Shape:
            x: (B, C, N, L), where C denotes in_dim
            h: (B, C', N, L'), where C' denotes h_dim
        """
...
class _DilatedInception(nn.Module):
    ...
    def __init__(
        ...
        if out_channels % n_kernels != 0:
            raise ValueError("out_channels` must be divisible by #kernels.")
        ...
        for k in kernel_size:
            self.convs.append(
                nn.Conv2d(in_channels=in_channels, out_channels=h_dim, kernel_size=(1, k), padding="same", dilation=dilation),
            )
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return torch.cat(x_convs, dim=1)
