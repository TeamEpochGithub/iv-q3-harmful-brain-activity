...
class SmoothPatient(VerboseTrainingBlock):
    ...
    smooth_factor: float = (
        0  # If this factor is 0,
        # the predictions will not be smoothed. 1 means the predictions will be fully smoothed based on the average of the patient's predictions.
    )
    ...
    def custom_train(self, x: npt.NDArray[np.float32], y: npt.NDArray[np.float32], **train_args: Any) -> tuple[Any, Any]:
        ...
        patient_predictions = {}  # type: ignore[var-annotated]
    ...
    def custom_predict(self, x: npt.NDArray[np.float32], **pred_args: Any) -> npt.NDArray[np.float32]:
        """Apply the smoothing to the predictions.
