...
class ExtractFeatures(VerboseTransformationBlock):
    ...
    def custom_transform(self, data: XData, **kwargs: Any) -> XData:
        ...
        features = []
        for i, row in tqdm(data.meta.iterrows(), desc="Extracting features"):
    ...
    def _extract_from_row(self, row: pd.Series) -> pd.Series:
        """Extract features from the EEG data.

        :param row: The row of metadata
        :return: The extracted features
        """
        raise NotImplementedError

    def _extract_from_sequence(self, eeg: pd.DataFrame, sampling_rate: int) -> pd.Series:
        """Extract features from the EEG data.
        ...
        normalized_amplitude_difference_index = abs(amplitude_left - amplitude_right) / (
